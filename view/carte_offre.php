				...
				<div class="flex w-full items-center justify-between">
					...
					<div class="flex gap-10 self-start items-center justify-center">
						...
						?>
							<a
								<?php
								// Check if 'rib' is not set or empty
								if ($pro['data']['type'] == 'prive' && (!isset($pro['data']['id_rib']) || $pro['data']['id_rib'] == null)) {
								?>
								href="/pro/compte/parametres"
								onclick="return confirm('Veuillez renseigner votre IBAN pour mettre <?php echo htmlspecialchars($titre_offre, ENT_QUOTES, 'UTF-8'); ?> en ligne')"
								title="Renseigner l'IBAN">
							<?php
								} else {
							?>
								href="/scripts/toggle_ligne.php?id_offre=<?php echo $id_offre ?>"
								onclick="return confirm('Voulez-vous vraiment mettre votre offre en ligne ? N\'hésitez pas à consulter de nouveau nos CGV')"
