...
Array.from(document.getElementsByClassName("eye-toggle-password")).forEach(
  (eye) => {
  ...
  Array.from(inputs).forEach((input) => {
    input.addEventListener("input", () => {
      const hasChanges = Array.from(inputs).some(
        (input, i) => input.value !== initialValues[i]
      );
...
function displayNotification(message) {
  ...
  toast.className =
    "fixed right-5 bg-secondary text-white p-4 z-50 border border-white flex items-center gap-4";
...
// Fonction pour visualiser une image unique dans les offres
function openImageModalOffre(carte_resto) {
  const modal = document.getElementById("imageModalOffre" + carte_resto);
  const image = document.getElementById("modalImageOffre" + carte_resto);
  if (modal && image) {
    modal.classList.remove("hidden");
    image.src = "public/images/offres/" + carte_resto;
  }
}
