class PrestationManager {
    ...
    if (this.prestationsContainer.size() === 0) {
      ...
      this.prestationsContainer["prestations"]
        ...
        .forEach((prestation) => {
          ...
          elementInclude.classList.add(
            "h-max",
            "w-full",
            "flex",
            "justify-center",
            "items-center",
            prestation.isIncluded ? "fill-secondary" : "text-rouge-logo",
            "",
            "bg-clip-content"
          );
  ...
  addPrestationsToForm() {
    ...
    document
      .querySelectorAll(".prestations-input")
