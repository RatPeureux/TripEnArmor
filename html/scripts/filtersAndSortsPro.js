document.addEventListener("DOMContentLoaded", function () {
  ...
  function filterOnOfferTypes(device) {
    const checkboxes = document.querySelectorAll(
      "#developped-f7-" + device + ' input[type="checkbox"]'
    );
  ...
  function applyFiltersPro() {
    ...
    offres?.forEach((offre) => {
      ...
      const type =
        offre
          ?.querySelector(".type-offre")
          ?.textContent.trim()
          ?.match(/Type : (Standard|Premium)/)[1]
          ?.toLowerCase() ?? null;
      ...
      if (type) {
        matchesType =
          filterState.types.length === 0 || filterState.types.includes(type);
      }
