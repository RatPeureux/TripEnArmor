            ...
            <div class="relative w-full max-w-96 h-fit flex flex-col items-center justify-center sm:w-96 m-auto">
                ...
                <form class="bg-base100 w-full p-5 rounded-lg border-2 border-secondary" action="create-pro.php"
                    ...
                    <div class="relative w-full">
                        ...
                        <i class="fa-regular fa-eye fa-lg absolute top-6 right-4 cursor-pointer bg-red-500"
    ...
    <body class="h-screen bg-white pt-4 px-4 overflow-x-hidden">
        ...
        <div class="w-full max-w-96 h-fit flex flex-col items-end sm:w-96 m-auto">
            ...
            <p class=" pb-3">Dites-nous en plus !</p>
                ...
                <!-- SEULEMENT POUR ORGANISME PUBLIC -->
                <?php
                if ($statut == 'public') { ?>
                    <!-- Choix du type d'organisation publique -->
                    <label class="text-small" for="statut">Je suis une&nbsp;</label>
                    <select class="text-small mt-1.5 mb-3 bg-white p-1 rounded-lg" id="type_orga" name="type_orga"
                        title="Choisir un statut" required>
                        <option value="" disabled selected> --- </option>
                        <option value="association">association</option>
                        <option value="orga_publique">organisation publique autre</option>
                    </select>
                    <br>
                    <?php
                }
                ...
                <?php if ($statut == "privé") { ?>
                    ...
                    <div class="group">
                        <div class="mb-1.5 flex items-start">
                            ...
                            <label class="text-small" for="plus">Je souhaite saisir mes informations bancaires dès maintenant
                                !</label>
                        </div>
...
<?php } else {
    ...
    try {
        $dbh->exec(statement: "ALTER TABLE sae_db._rib ALTER COLUMN numero_compte TYPE VARCHAR(12)");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['num_tel'])) {
        ...
        if ($stmtAdresse->execute()) {
            ...
            if ($statut === "public") {
                ...
                $stmtPublic = $dbh->prepare("INSERT INTO sae_db._pro_public (email, mdp_hash, num_tel, adresse_id, nompro, type_orga) VALUES (:mail, :mdp, :num_tel, :adresse_id, :nom, :type_orga)");

                // Lier les paramètres pour le professionnel
                $stmtPublic->bindParam(':nom', $nom);
                $stmtPublic->bindParam(':mail', $mail);
                $stmtPublic->bindParam(':mdp', $mdp_hash);
                $stmtPublic->bindParam(':num_tel', $tel);
                $stmtPublic->bindParam(':adresse_id', $adresseId);
                $stmtPublic->bindParam(':type_orga', $type_orga);

                // Exécuter la requête pour le professionnel public
                $stmtPublic->execute();
            }
            // Pour le privé (IN WORK)
            else {

                // if ($stmtPrive->execute()) {
                ...
                //     }
                // } else {
                //     $message = "Erreur lors de la création du compte professionnel public : " . implode(", ", $stmtPrive->errorInfo());
                // }
            }
        } else {
...
<script>
    ...
    // Boutons pour montrer / cacher les mots de passe
    if (togglePassword1) {
