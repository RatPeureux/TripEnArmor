...
<body class="min-h-screen flex flex-col justify-between">
    ...
    if (isset($_COOKIE['recentes'])) {
        // Transformer le tableau en liste d'IDs pour la requête SQL (ex: "1,2,3")
        $placeholders = implode(', ', json_decode($_COOKIE['recentes']));
        ...
        ");
        $stmt->execute();
        $consulteesRecemments = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if (isset($_GET['sort'])) {
        ...
        if ($_GET['sort'] === 'note-ascending') {
            ...
            usort($consulteesRecemments, function ($a, $b) {
    ...
    foreach ($consulteesRecemments as $offre) {
        ...
        <div class="flex justify-center w-full md:max-w-[1280px]">
            ...
            <main class="grow p-4 md:p-2 flex flex-col md:mx-10">
                ...
                <div class="flex justify-between items-end mb-2">
                    <h1 class="text-3xl ">Consultées récemments</h1>
                ...
                <?php } else { ?>
                    <div class="md:min-w-full flex flex-col gap-4" id="no-matches">
                        <?php foreach ($consulteesRecemments as $offre) {
