...
CREATE TABLE _RIB (
    ...
    compte_id INTEGER REFERENCES _pro_prive(id_compte) UNIQUE
);
...
CREATE TABLE _tag_offre (
    offre_id INTEGER REFERENCES _offre(offre_id),
    tag_id INTEGER REFERENCES _tag(tag_id),
...
create table _tag_restaurant_restauration (
  restauration_id integer references _restauration(restauration_id),
  tag_restaurant_id integer references _tag_restaurant(tag_restaurant_id),
...
create table _tag_activite (
  id_active integer references _activite(id_active),
  tag_id integer references _tag(tag_id),
...
create table _tag_spectacle (
  spectacle_id integer references _spectacle(spectacle_id),
  tag_id integer references _tag(tag_id),
...
create table _tag_visite (
  visite_id integer references _visite(visite_id),
  tag_id integer references _tag(tag_id),
...
CREATE TABLE _visite_langue (
    visite_id INTEGER REFERENCES _visite(visite_id),
    langue_id INTEGER REFERENCES _langue(langue_id)
);
...
create table _tag_parc (
  parc_id integer references _parc_attraction(parc_id),
  tag_id integer references _tag(tag_id),
